...
def editBook(request):
    ...
    res=render(request,'BRMapp/edit_boook.html',{'form':form,'book':book})
    return res
def edit(request):
...
def add(request):
    if request.method=='POST':
        form=NewBookForm(request.POST) #data coming from form
        book=models.Book()        #object of class Book in models.py
        book.title=form.data['title'] #way to accesss data present in the form
        book.price=form.data['price']
        book.author=form.data['author']
        book.publisher=form.data['publisher']
        book.save()
